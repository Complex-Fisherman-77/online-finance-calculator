...
export const formatCurrency = (value: number, locale: string = 'en'): string => {
  if (value === 0) return '';
  return new Intl.NumberFormat(locale, currencyOptions[locale as keyof typeof currencyOptions]).format(value);
};

// Format percentage based on locale
export const formatPercentage = (value: number, locale: string = 'en'): string => {
  if (value === 0) return '';
  return new Intl.NumberFormat(locale, percentageOptions[locale as keyof typeof percentageOptions]).format(value);
};
